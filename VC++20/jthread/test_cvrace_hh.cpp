...
namespace {
    ...
    void testCVAnyMutex()
    {
        ...
        void* raw = malloc(sizeof(std::condition_variable_any));
        cv = new(raw) std::condition_variable_any;
        std::thread th2(g);
        m.lock();
        while (!g_ready)
            cv->wait(m);
    ...
    void testCVAnyMutex()
    {
        ...
        void* raw = malloc(sizeof(std::condition_variable_any));
        std::condition_variable_any* cv = new(raw) std::condition_variable_any;
        std::mutex m;
        bool f_ready = false;
        bool g_ready = false;
